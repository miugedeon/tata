...
?>
    <li class="p1"><a <?php if($row[p_id]==$typeID) echo "class='hover'" ?> href="<?php echo $p ?>-<?php echo $row[p_id] ?>.html" target="_self"><?php echo $row[p_title_en]; ?></a></li>
	<?php
    $sql2="select * from p_product_type where language='cn' and p_bigtype=".$row[p_id];
    $query2=$d->Query($sql2);
    while($row2=mysql_fetch_array($query2)){
    ?>
    <li class="p2"><a <?php if($row2[p_id]==$twoID) echo "class='hover'" ?> href="<?php echo $p ?>-<?php echo $row2[p_bigtype] ?>-<?php echo $row2[p_id] ?>.html" target="_self"><?php echo $row2[p_title_en]; ?></a></li>
    <?php
    }
    ?>
<?php
