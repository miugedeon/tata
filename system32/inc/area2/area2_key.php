...
<input type="hidden" name="language" id="language" value="<?php echo $language?>">
			<?php
				$key=$_GET["key"];
            	$sql="select * from p_area2 where language='".$language."' and p_productname like '%" .$key. "%'";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
					...
					$sql="select * from p_area2 where language='".$language."' and p_productname like '%" .$key. "%' order by p_sort desc limit ".$offset.",".$pagesize."";
            	...
            	<td colspan="11" align="center">
                	<table cellpadding="0" id="page" cellspacing="0" border="0" style="height:40px; line-height:40px; color:#333333;">
                    	<tr>
                            ...
                            <td width="10"></td>
                            <td><a href="area2_manage.php?language=<?php echo $language?>&action=p_key&key=<?php echo $key?>&page=<?php echo 1?>">首页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?language=<?php echo $language?>&action=p_key&key=<?php echo $key?>&page=<?php echo $page-1?>">上页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?language=<?php echo $language?>&action=p_key&key=<?php echo $key?>&page=<?php echo $page+1?>">下页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?language=<?php echo $language?>&action=p_key&key=<?php echo $key?>&page=<?php echo $pagecount?>">末页</a></td>
                            <td width="5"></td>
                            <td>转:</td>
                            <td>
...
<script language="javascript">
	...
	function GoPage()
	{
		...
		window.location.href="area2_manage.php?language=<?php echo $language?>&action=p_key&key=<?php echo $key?>&page="+goPage+"";
	}
</script>            