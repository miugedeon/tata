			...
			<?php
            	...
            	$sql="select * from p_area2 where language='".$language."' and p_smalltype=".$p_smalltype."";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
					...
					$sql="select * from p_area2 where language='".$language."' and p_smalltype=".$p_smalltype." order by p_sort desc limit ".$offset.",".$pagesize."";
            ...
            <tr onMouseOut="this.className='out'" style="height:22px; line-height:22px;">
                ...
                <td align="center" class="c_666">
               		 <?php
                     	$sql="select * from p_area2_type where p_id=".$row["p_bigtype"]."";
						$big=$d->Query($sql);
                     ...
                     <?php
                     	$sql="select * from p_area2_type where p_id=".$row["p_smalltype"]."";
						$small=$d->Query($sql);
                ...
                <td width="8%" align="center"><a href="area2_delete_one.php?p_id=<?php echo $row["p_id"]?>&action=1&language=<?php echo $language?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="area2_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>">更新</a></td>
            </tr>
            	...
            	<td colspan="11" align="center">
                	<table cellpadding="0" id="page" cellspacing="0" border="0" style="height:40px; line-height:40px; color:#333333;">
                    	<tr>
                            ...
                            <td width="10"></td>
                            <td><a href="area2_manage.php?p_bigtype=<?php echo $p_bigtype?>&action=<?php echo $_GET["action"]?>&language=<?php echo $_GET["language"]?>&p_smalltype=<?php echo $_GET["p_smalltype"]?>&page=<?php echo 1?>">首页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?p_bigtype=<?php echo $p_bigtype?>&action=<?php echo $_GET["action"]?>&language=<?php echo $_GET["language"]?>&p_smalltype=<?php echo $_GET["p_smalltype"]?>&page=<?php echo $page-1?>">上页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?p_bigtype=<?php echo $p_bigtype?>&action=<?php echo $_GET["action"]?>&language=<?php echo $_GET["language"]?>&p_smalltype=<?php echo $_GET["p_smalltype"]?>&page=<?php echo $page+1?>">下页</a></td>
                            <td width="5"></td>
                            <td><a href="area2_manage.php?p_bigtype=<?php echo $p_bigtype?>&action=<?php echo $_GET["action"]?>&language=<?php echo $_GET["language"]?>&p_smalltype=<?php echo $_GET["p_smalltype"]?>&page=<?php echo $pagecount?>">末页</a></td>
                            <td width="5"></td>
                            <td>转:</td>
                            <td>
...
<script language="javascript">
	...
	function GoPage()
	{
		...
		window.location.href="area2_manage.php?p_bigtype=<?php echo $p_bigtype?>&action=<?php echo $_GET["action"]?>&language=<?php echo $_GET["language"]?>&p_smalltype=<?php echo $_GET["p_smalltype"]?>&page="+goPage+"";
	}
</script>            