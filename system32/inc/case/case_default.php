<?php
	$sql="select * from p_case where language='".$language."' and p_typelist='".$p_typelist."'";
	$num=$d->Execute($sql);
	if($num>0)
	{
		$pagesize=12;
		...
		$sql="select a.*,b.p_title as btitle from p_case a left join p_list b on b.p_id=a.p_type where a.language='".$language."' and a.p_typelist='".$p_typelist."' order by a.p_id desc limit ".$offset.",".$pagesize."";
...
<tr style="height:22px; line-height:22px;">
	...
	<td align="center" width="10%" class="c_333"><?php echo $row["btitle"]?></td>
	...
	<td align="center" width="7%"><a href="case_delete_one.php?p_id=<?php echo $row["p_id"]?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="case_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>&p_typelist=<?php echo $p_typelist?>">更新</a></td>
</tr>
<?php
		}
?>
