<?php
	$sql="select * from p_cs order by p_type asc";
	$num=$d->Execute($sql);
	if($num>0)
	{
		$pagesize=12;
...
<tr style="height:22px; line-height:22px;">
	...
	<td align="center" width="35%" class="c_333"><?php echo $row["p_sq"]?><?php echo $row["p_ww"]?></td>
	<td align="center" width="8%"><?php if($row["p_type"]==0){?><font color="#CC0033">授权编号</font><?php }elseif($row["p_type"]==1){?><font color="#0000FF">店铺掌柜</font><?php } ?></td>
	<td align="center" width="7%"><a href="cs_delete_one.php?p_id=<?php echo $row["p_id"]?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="cs_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>&p_typelist=<?php echo $p_typelist?>">更新</a></td>
</tr>
<?php
		}
?>
