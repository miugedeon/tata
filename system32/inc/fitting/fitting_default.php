			...
			<?php
            	$sql="select * from p_fitting where language='".$language."'";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
					...
					$sql="select * from p_fitting where language='".$language."' order by p_id asc limit ".$offset.",".$pagesize."";
            ...
            <tr onMouseOut="this.className='out'" style="height:22px; line-height:22px;">
                ...
                <td width="8%" align="center"><a href="fitting_delete_one.php?p_id=<?php echo $row["p_id"]?>&action=1&language=<?php echo $language?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="fitting_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>">更新</a></td>
            </tr>
...
<script language="javascript">
	...
	function GoPage()
	{
		...
		window.location.href="fitting_manage.php?page="+goPage+"&language=<?php echo $language?>";
	}
