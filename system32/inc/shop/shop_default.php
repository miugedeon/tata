<?php
            	$sql="select * from p_shop where language='".$language."'";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
					...
					$sql="select * from p_shop where language='".$language."' order by p_ordering desc,p_id desc limit ".$offset.",".$pagesize."";
            ...
            <tr onMouseOut="this.className='out'" style="height:22px; line-height:22px;">
                ...
                <td width="15%" align="center" class="c_666" height="41"><img src="<?php echo $row["p_piclink"]?>" width="80" height="40" border="0"/></td>
                 
                <td width="5%" align="center" class="c_666"><?php echo $row["p_title"]?></td>
                
                <td align="center" class="c_666"><?php echo $row["p_link"]?></td>  
                
                <td align="center" class="c_666"><?php echo $row["p_price1"]?></td>  
                <td align="center" class="c_666"><?php echo $row["p_price2"]?></td>  
                <td align="center" class="c_666"><?php echo $row["p_ordering"]?></td>  
                       
                <td width="8%" align="center"><a href="shop_delete_one.php?p_id=<?php echo $row["p_id"]?>&action=1&language=<?php echo $language?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="shop_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>">更新</a></td>
            </tr>
