<?php
            	$sql="select * from p_special order by p_id desc";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
            ...
            <tr onMouseOut="this.className='out'" style="height:22px; line-height:22px;">
                ...
                <td align="center" class="c_666" height="41"><?php echo $row["p_id"]?></td>
                 
                <td align="center" class="c_666"><?php echo $row["p_title"]?></td>
                
                <td align="center" class="c_666"><?php echo $row["p_station_1"]?></td> 
                       
                <td width="8%" align="center"><a href="special_delete_one.php?p_id=<?php echo $row["p_id"]?>&action=1&language=<?php echo $language?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="../special/<?php echo $row["p_id"]?>" target="_blank">预览</a>|<a href="special_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>">更新</a></td>
            </tr>
