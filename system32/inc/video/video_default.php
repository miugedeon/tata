			...
			<?php
            	$sql="select * from p_video where language='".$language."'";
				$num=$d->Execute($sql);
				if($num>0)
				{
					$pagesize=11;
					...
					$sql="select * from p_video where language='".$language."' order by p_ordering asc, p_id asc limit ".$offset.",".$pagesize."";
            ...
            <tr onMouseOut="this.className='out'" style="height:22px; line-height:22px;">
                ...
                <td align="center" class="c_666"><?php echo $row["p_id"]?></td>
                
                <td align="center" class="c_666"><?php echo $row["p_title"]?></td>
                <td align="center" class="c_666"><?php echo $row["p_title_en"]?></td>
                
                <td align="center" class="c_666"><?php echo $row["p_ordering"]?></td>
                         
                <td align="center"><a href="video_delete_one.php?p_id=<?php echo $row["p_id"]?>&action=1&language=<?php echo $language?>" onClick="return confirm('您确认删除吗？')">删除</a>|<a href="video_update.php?p_id=<?php echo $row["p_id"]?>&language=<?php echo $language?>">更新</a></td>
            </tr>
...
<script language="javascript">
	...
	function GoPage()
	{
		...
		window.location.href="video_manage.php?page="+goPage+"&language=<?php echo $language?>";
	}
